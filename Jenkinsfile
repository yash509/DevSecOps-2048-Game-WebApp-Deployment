pipeline {
    ...
    environment {
        IMAGE_NAME = "yash5090/twozerofoureight"
    ...
    stages {
        ...
        stage('Checkout from Git') {                        
            steps {                                       
                git branch: 'main', url: 'https://github.com/yash509/DevSecOps-twozerofoureight-WebApp-Deployment.git'
            }
        }
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop twozerofoureight | true"
                sh "docker rm twozerofoureight | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh "docker run -d --name twozerofoureight -p 3000:3000 ${IMAGE_NAME}:${TAG}" 
                //}
            }
        }
        ...
        stage('Switch Traffic Between Blue & Green Environment') {
            ...
            steps {
                script {
                    ...
                    withKubeConfig(caCertificate: '', clusterName: '', contextName: '', credentialsId: 'k8s', namespace: '', restrictKubeConfigAccess: false, serverUrl: '') {
                        sh '''
                            kubectl patch service twozerofoureight-service -p "{\\"spec\\": {\\"selector\\": {\\"app\\": \\"twozerofoureight\\", \\"version\\": \\"''' + newEnv + '''\\"}}}"
                        '''
                    }
